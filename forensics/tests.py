...
class AuditLogIngestionTests(TestCase):
    ...
    def test_unicode_line_separator_inside_json_string_does_not_split_jsonl_record(self):
        ...
        separator = "\u2028"
        first_event = audit_event(0)
        first_event["context"]["operation_id"] = f"before{separator}after"
        first_line = json.dumps(first_event, ensure_ascii=False, separators=(",", ":"))
        second_line = json.dumps(audit_event(1), ensure_ascii=False, separators=(",", ":"))
        body = f"{first_line}\n{second_line}\n"
        ...
        events = list(AuditEvent.objects.order_by("line_number"))
